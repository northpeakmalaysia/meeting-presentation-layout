...
import type {
  MeetingHubPlugin,
  PresentationCloseInput,
  PresentationNavigateInput,
  PresentationOpenInput,
  PresentationOpenResult,
} from '@swarmai/meeting-hub-sdk';

interface PresentationLayoutConfig {
  /** PNG render width in pixels; default 1280. */
  renderWidth?: number;
  /** PNG quality 0..1; pdfjs uses scale, we map quality to scale. */
  scale?: number;
  /** When true, also render thumbnails for a filmstrip. */
  renderThumbnails?: boolean;
  /** Override path/name of the LibreOffice binary used for PPTX. */
  libreOfficeBin?: string;
  /** Cap pages — no presentation longer than this; protects against runaway renders. */
  maxPages?: number;
}

export const presentationLayout: MeetingHubPlugin = {
  manifest: {
  ...
  async onPresentationOpen(input: PresentationOpenInput): Promise<PresentationOpenResult> {
    const cfg = input.config as PresentationLayoutConfig;
    ...
    for (let i = 1; i <= totalPages; i++) {
      ...
      pageUrls.push(
        `/m/${input.meetingId}/presentations/${input.artefactId}/page/${i}.png`,
      );
    }
  ...
  async onPresentationNavigate(input: PresentationNavigateInput): Promise<void> {
  ...
  async onPresentationClose(input: PresentationCloseInput): Promise<void> {
...
async function convertPptxToPdf(
  ...
  if (!exists) throw new Error('LibreOffice ran but produced no PDF');
  return fs.readFile(outputPath);
}

function defaultLibreOfficeBin(): string {
